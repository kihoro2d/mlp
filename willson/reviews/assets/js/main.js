...
new Vue({
  data: {
    config: {
      shown: 2,    // кол-во отображаемых отзывов
      bunch: 5,    // кол-во подгружаемых отзывов
      delay: 5000, // задержка добавления отзыва (1000 = 1 секунда)

      fake: {
        rating: 4.5, // средний рейтинг ложных отзывов
        quantity: 20 // количество ложных отзывов
      },
  ...
  methods: {
    ...
    mediaFiles(files) {
      return files.map(file => ({ thumb: file, src: file }))
    },
    ...
    onSubmitAnswerForm(event) {
      ...
      if (elems.email.value.length < 3)
        return alert('Введите электронную почту')
  ...
  async created() {
    if (new URLSearchParams(window.top.location.search).has('_rev_reset')) {
      this.$storage.clear()
    }
    ...
    this.reviews = await this.$storage.remember('reviews', async () => {
